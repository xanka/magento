<?php
/**
 * @copyright   Copyright (c) 2010 SM
 */ 
class SM_Base_Block_Extensions extends Mage_Adminhtml_Block_System_Config_Form_Fieldset
{
    ...
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        foreach ($modules as $moduleName) {
        	if (strstr($moduleName, 'SM_') === false) {
        		continue;
        	}
			
			if ($moduleName == 'SM_Base'){
				continue;
			}
	...
	protected function _getFieldHtml($fieldset, $moduleCode)
    {
		...
		$allExtensions = unserialize(Mage::app()->loadCache('smbase_extensions'));
            
        $status = '<a  target="_blank"><img src="'.$this->getSkinUrl('images/smbase/ok.gif').'" title="'.$this->__("Installed").'"/></a>';
		...
		if ($allExtensions && isset($allExtensions[$moduleCode])){
            ...
            if ($this->_convertVersion($currentVer) < $this->_convertVersion($lastVer)){
                $status = '<a href="'.$url.'" target="_blank"><img src="'.$this->getSkinUrl('images/smbase/update.gif').'" alt="'.$this->__("Update available").'" title="'.$this->__("Update available").'"/></a>';
            }
        }
