<?php
class SM_Vendors_Model_Override_Sales_Order_Pdf_Creditmemo extends SM_Vendors_Model_Override_Sales_Order_Pdf_Abstract
{
    public function getPdf($creditmemos = array())
    {
        if(!Mage::helper('smvendors')->useCustomPdf()) { // Old Magento Pdf
            return $this->_getPdf($creditmemos);
        } else {
            ...
            foreach ($creditmemos as $creditmemo) {
                ...
                $vendor = Mage::getModel('smvendors/vendor')->load($creditmemo->getVendorId());
                $vendorOrder = Mage::getModel('smvendors/order')->getByOriginOrderId($order->getId(), $vendor->getId());
    
                $params = array(
                        'template' => 'sm/vendors/pdf/creditmemo.phtml',
    ...
    public function _getPdf($creditmemos = array())
    {
        $this->_beforeGetPdf();
