 ...
 */
class SM_Vendors_Model_Resource_Reports_Customer_Collection extends Mage_Reports_Model_Resource_Customer_Collection
{
    /**
    ...
    public function joinOrders($from = '', $to = '')
    {
        ...
        $this->getSelect()
            ->joinLeft(array('orders' => $this->getTable('smvendors/order')),
    ...
    public function addOrdersCount()
    {
        $this->getSelect()
            ...
            ->where('orders.status <> ?', Mage_Sales_Model_Order::STATE_CANCELED)
            ->group("e.entity_id");
    ...
    protected function _addOrdersStatistics()
    {
        ...
        if ($this->_addOrderStatistics && !empty($customerIds)) {
            ...
            $select->from(array('orders'=>$this->getTable('smvendors/order')), array(
                'orders_avg_amount' => "AVG({$totalExpr})",
                'orders_sum_amount' => "SUM({$totalExpr})",
                'orders_count' => 'COUNT(orders.entity_id)',
                'customer_id' => 'po.customer_id'
            ))->where('orders.status <> ?', Mage_Sales_Model_Order::STATE_CANCELED)
              ->where('po.customer_id IN(?)', $customerIds)
              ->group('po.customer_id');

        	$mainTableOrder = $this->getTable('sales/order');
    	 	$select->join(array('po' => $mainTableOrder),'orders.order_id = po.entity_id',null);
        
	        if($vendor = Mage::helper('smvendors')->getVendorLogin()){
	        	$select->where('orders.vendor_id = ?',$vendor->getId());
	        }  
	...
	public function joinVendorCustomer(){
        ...
        if($vendor = Mage::helper('smvendors')->getVendorLogin()){
        	$mainTableOrder = $this->getTable('smvendors/vendor_customer');
    		$this->getSelect()
           		->join(array('vc' => $mainTableOrder),'vc.customer_id = e.entity_id',null)
           		->where('vc.vendor_id = ?',$vendor->getId())
           		->group('e.entity_id');
        }
