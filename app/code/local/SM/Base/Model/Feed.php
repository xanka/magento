<?php
/**
 * @copyright  Copyright (c) 2010 SM (http://www.amasty.com)
 */  
class SM_Base_Model_Feed extends Mage_AdminNotification_Model_Feed
{
    const XML_FREQUENCY_PATH    = 'smbase/feed/check_frequency';
    const XML_LAST_UPDATE_PATH  = 'smbase/feed/last_update';
    const XML_ITERESTS          = 'smbase/feed/interests';
	...
	{
		return Mage::getModel('smbase/feed')->checkUpdate();
	}
    ...
    public function checkUpdate()
    {
        ...
        $wasInstalled = gmdate('Y-m-d H:i:s', Mage::getStoreConfig('smbase/feed/installed'));
        ...
        if ($feedXml && $feedXml->channel && $feedXml->channel->item) {
            ...
            if ($feedData) {
                Mage::app()->saveCache(serialize($feedData), 'smbase_extensions');
            }
        }
    ...
    {
        return Mage::app()->loadCache('smbase_notifications_lastcheck');
    }
 
    public function setLastUpdate()
    {
        Mage::app()->saveCache(time(), 'smbase_notifications_lastcheck');
        return $this;
    }
	...
	protected function isInteresting($item)
	{
		...
		$selfUpgrades = array_search(SM_Base_Model_Source_Updates_Type::TYPE_INSTALLED_UPDATE, $types);
		...
		foreach ($types as $type){
			...
			if ($extenion && ($type == SM_Base_Model_Source_Updates_Type::TYPE_UPDATE_RELEASE) && $selfUpgrades){
